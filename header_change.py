...
# 多groups时使用
GIT_REPO_GROUPS = {'appdemo2':'group2'}
GIT_DEFAULT_GROUP = 'group1'
# 需要更改的pod名称 
CHECK_PODS = []
...
def clone_repo(group,module):
    print module
    p = subprocess.Popen(['git', 'clone', '-b', 'develop' ,'--progress', 'git@git.xxx.net:' + group + '/' + module + '.git'], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    while True:
        output = p.stdout.readline()
        if output == '' and p.poll() is not None:
            break
...
def clone_repos(modules):
    for module in modules:
        if module in GIT_REPO_GROUPS.keys():
            ...
            clone_repo(GIT_DEFAULT_GROUP,module)
            pass
